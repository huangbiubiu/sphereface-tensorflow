...
def Loss_ASoftmax(x, y, l, num_cls, m=4, name='asoftmax'):
    """
    A Softmax by https://github.com/DQ0408/SphereFace-TensorFlow/blob/master/model_and_train.py
    x: B x D - data
    y: B x 1 - label
    l: 1 - lambda
    """
    ...
    if m == 1:
        ...
        comb_logits_diff = tf.add(logits,
                                  tf.scatter_nd(ordinal_y, tf.subtract(scaled_logits, sel_logits), logits.get_shape()))
